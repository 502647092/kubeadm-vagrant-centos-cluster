...
Vagrant.configure("2") do |config|
  ...
  (1..$node_num).each do |i|
    config.vm.define "#{$name_prefix}#{i}" do |node|
      node.vm.box = "centos/7"
      node.vm.hostname = "#{$name_prefix}#{i}"
      ip = "192.168.25.#{i+100}"
...
echo "Init Node${NODE} Base Env..."
ln -s /usr/share/zoneinfo/Asia/Shanghai /etc/localtime
timedatectl set-timezone Asia/Shanghai
cp /vagrant/yum/CentOS7-Base-163.repo /etc/yum.repos.d/CentOS-Base.repo
yum install -y wget
